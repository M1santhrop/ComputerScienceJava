...
public class Album extends DataPoint {
    ...
    public static void main(String[] args) {
        ...
        List<KMeans<Album>.Cluster> albumClusters = kMeans.run(100);
        for (int clusterIndex = 0; clusterIndex < albumClusters.size(); clusterIndex++) {
            System.out.printf("Cluster %d Avg Length %f Avg Tracks %f: %s%n",
                    clusterIndex,
                    albumClusters.get(clusterIndex).centroid.dimensions.get(0),
                    albumClusters.get(clusterIndex).centroid.dimensions.get(1),
                    albumClusters.get(clusterIndex).points);
        }
    }
}
